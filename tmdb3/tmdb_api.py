...
class MovieSearchResult(SearchRepr, PagedRequest):
    ...
    def __init__(self, request, locale=None):
        if locale is None:
            locale = get_locale()
        super(MovieSearchResult, self).__init__(request.new(language=locale.language),
...
class PeopleSearchResult(SearchRepr, PagedRequest):
    ...
    def __init__(self, request):
        super(PeopleSearchResult, self).__init__(request, lambda x: Person(raw=x))
...
class StudioSearchResult(SearchRepr, PagedRequest):
    ...
    _name = None
    def __init__(self, request):
        super(StudioSearchResult, self).__init__(request, lambda x: Studio(raw=x))
...
class CollectionSearchResult(SearchRepr, PagedRequest):
    ...
    def __init__(self, request, locale=None):
        if locale is None:
            locale = get_locale()
        super(CollectionSearchResult, self)\
              .__init__(request.new(language=locale.language),
...
class Person(Element):
    ...
    profile = Datapoint('profile_path', handler=Profile, raw=False, default=None)
...
class Movie(Element):
    ...
    @property
    def similar(self):
        res = MovieSearchResult(Request('movie/{0}/similar_movies'.format(self.id)),
                                locale=self._locale)
